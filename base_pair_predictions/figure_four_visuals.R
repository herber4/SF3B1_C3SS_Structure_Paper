...
setwd("/Desktop/Figures/Figure_4/data/")

all_random_ag_shape <- read.table(file = "All_SHAPE_Random_AG_for_linegraphs.txt",
                                  sep = "\t", header = TRUE)

mas <- read.table(file = "shape_vs_no_shape_bpp_k700e/K700E_bpp_SHAPE_both_MDs.txt",
                  sep = "\t", header = TRUE)
ses <- mas
ses <- ses %>%
  filter(source == "C3SS",
...
result$plot <- "Random_AG_inside_SJs"
result <- result %>%
  filter(Sequence == "G")
...
con <- read.table(file = "/Desktop/Figures/Figure_4/data/shape_vs_no_shape_controls/Control_SHAPE_bpp_indexed_for_line_graphs.txt",
                  sep = "\t", header = TRUE)
con[,c(3,8)] <- NULL
...
mas <- rbind(mas, con)
rm(con)
mas <- mas[,c(1, 2, 5, 8, 11, 12)]
all_random_ag_shape <- all_random_ag_shape[,c(10:13, 7, 3)]
colnames(all_random_ag_shape)[2] <- "gene_name"
colnames(all_random_ag_shape)[4] <- "new_plot"

mas <- rbind(mas, all_random_ag_shape)

prof_mean <- mas %>%
  filter(new_plot == "Short_MD_OutSJs_randomAG")
...
mas %>%
  filter(new_plot %in% c("SS_AG:MT_Can_md28_to_34",
                         "SS_AG:MT_C3SS_md28_to_34",
                         "Short_MD_OutSJs_randomAG")) %>%
  ggplot(aes(x = as.factor(new_index), y = 1 - av_sum_p, fill = new_plot)) +
  geom_boxplot()

values <- c("SS_AG:MT_Can_md28_to_34" = "#ae017e",
            "SS_AG:MT_C3SS_md28_to_34" = "#fbb4b9",
            "Short_MD_OutSJs_randomAG" = "black")

pdf(file = "MT_Accessibility_linegraph.pdf",
...
ggplot(stats %>%
  ...
  geom_hline(yintercept = 0.46, linetype = "dashed", color = "black") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  ylim(0,1) +
  scale_color_manual(values = values)
dev.off()
pdf(file = "MT_SHAPE_linegraph.pdf",
    width = 8, height = 4, paper = "letter")
ggplot(stats %>%
         filter(new_plot %in% c("SS_AG:MT_Can_md28_to_34",
                                "SS_AG:MT_C3SS_md28_to_34",
